...
```{r}
# =========================
# ОКС: сбор датасета + death_green (чистый XML)
# =========================
...
# XML-логика для определения зелёного шрифта (лежит в корне проекта)
source("death_green_xml.R")
...
process_file <- function(file_path, sheet_name, year_mode = c("filename", "admission"), sheet_id = 1) {
  ...
  # --- NEW: фильтрация по непустому ФИО (вместо "непустая строка") ---
  ...
  # если ФИО не нашли — не роняем пайплайн, просто возвращаем NULL
  if (is.na(fio_col_idx)) return(NULL)

  keep <- !is.na(raw_data[[fio_col_idx]]) & stringr::str_trim(raw_data[[fio_col_idx]]) != ""

  kept_rows_excel <- which(keep) + 1L  # реальные номера строк Excel (header = row 1)
...
result <- tibble(
  ...
  result <- add_col(result, data, "admissionDateTime",
                    c("дата и время госпитализации", "дата госпитализации", "ДАТА ГОСПИТАЛИЗАЦИИ", "дата госпитал-и"))
  result <- add_col(result, data, "ICU_EndDateTime",
  ...
  # death_green: чистый XML (цвет шрифта в колонке ФИО)
  result$death_green <- tryCatch(
    calc_death_green_xml(file_path, sheet_name, fio_col_idx, kept_rows_excel),
    error = function(e) rep(0L, nrow(data))
  )
